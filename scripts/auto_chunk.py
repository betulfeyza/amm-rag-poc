...
-------------------------------
İşlenen sayfalar:
  - 06___086.PDF  → Sayfalar 9, 10, 11  (PDF indeksleri: 8, 9, 10)
  - 20___086.PDF  → Sayfa 50             (PDF indeksi: 49)
  - 20___086.PDF  → Sayfa 59             (PDF indeksi: 58)
...
def resolve_raw_dir(base_dir: Path) -> Path:
    candidates = [
...
def main() -> None:
    ...
    # -- Sayfa tanımları ------------------------------------------------------
    # (pdf_name, page_idx, fallback_label)
    page_jobs = [
        ("06___086.PDF", 8,  "9"),
        ("06___086.PDF", 9,  "10"),
        ("06___086.PDF", 10, "11"),
    ...
    doc_dfs: dict[str, pd.DataFrame] = {}
    camelot_dfs: list[pd.DataFrame] = []
    auto_id = 1
    missing_pdfs: set[str] = set()

    for pdf_name, page_idx, fallback in page_jobs:
        pdf_path = RAW_DIR / pdf_name
        print(f"\n[{pdf_name}] Sayfa {page_idx + 1} işleniyor")

        if not pdf_path.exists():
            if pdf_name not in missing_pdfs:
        ...
        try:
            ...
            print(f"  ⚠ Sayfa işlenemedi ({pdf_name} / {page_idx + 1}): {exc}")
            continue
        ...
        if pdf_name in doc_dfs:
            doc_dfs[pdf_name] = pd.concat([doc_dfs[pdf_name], df], ignore_index=True)
        else:
            doc_dfs[pdf_name] = df
        auto_id += len(blocks)
    ...
    if not doc_dfs:
        print("\n❌ İşlenecek veri bulunamadı. Hiçbir PDF/sayfa işlenemedi.")
        if missing_pdfs:
            print(f"Beklenen konum: {RAW_DIR}")
            print("Eksik PDF'ler:")
            for pdf_name in sorted(missing_pdfs):
                print(f"  - {pdf_name}")
        return
    ...
    with pd.ExcelWriter(str(OUTPUT_FILE), engine="openpyxl") as writer:
        for pdf_name, df in doc_dfs.items():
            sheet_name = Path(pdf_name).stem[:31]
    ...
    for pdf_name in list(doc_dfs.keys()):
        sname = Path(pdf_name).stem[:31]
